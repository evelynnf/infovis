...
function YAxis (props) {
    const { yScale, height=-350, offsetX } = props;
    return <g className="y-axis">
        {<line x1={0} y1={0} x2={0} y2={-350} stroke='black'/>}
        {yScale.domain().map(tickValue => 
            <g key={tickValue} className="tick">
                <line x1={-5} x2={0} y1={yScale(tickValue) + yScale.bandwidth()/2} y2={yScale(tickValue) + yScale.bandwidth()/2} stroke='black' />
                {/* <text style={{ textAnchor:'start', fontSize:'10px' }} x={-offsetX+10} y={yScale.bandwidth()/2}>
                    {tickValue}
                </text> */}
                <text style={{textAnchor: 'start', fontSize:'10px'}} x={-130} y={yScale(tickValue) + yScale.bandwidth()/2 + 5}>
                    {tickValue}
                </text>
            </g>
        )}
...
function XAxis(props) {
    const { xScale, width=400, height} = props;
