...
export function BarChart (props) {
    ...
    const yScale = scaleBand().range([0, height]).domain(data.map(a => a.Year)).padding(0.2) //The domain is the list of ailines names
    let color = (d) => d.Year===selectedYear? "#992a5b":"#2a5599";
    let onClick = (d) => {
    ...
    return <g transform={`translate(${offsetX}, ${offsetY})`}>
        { data.map( d => {
            return <rect key={d.Year} x={0} y={yScale(d.Year)}
